...
{   
    ...
    int TrackingID;
    int ClipedEdge = 0;
    int HandLeftConfidence = 1;
    int HandLeftState = 1;
    int HandRighttConfidence = 1;
    int HandRighttState = 1;
    int isResticted = 0;
    float LeanX, LeanY;
    int TrackingState;
    int NumJoints = 25;
    ...
    */
    ...
    int HandRighttState = 1;
    int isResticted = 0;
    int TrackingState = K4ABT_JOINT_CONFIDENCE_MEDIUM;
    int NumJoints = 25;
    float LeanX, LeanY;
    k4abt_body_t body;
...
bool ParseInputSettingsFromArg(int argc, char** argv, InputSettings& inputSettings)
{
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (inputArg == std::string("SAVE"))
        {
            inputSettings.SaveSkeleton = true;
            if (i < argc - 1) {
                // Take the next argument after SAVE as file name
                inputSettings.OutName = argv[i + 1];
                i++;
            }
            else {
                return false;
...
void PlayFile(InputSettings inputSettings) {
    ...
    // Skeleton Save
    int num_frames = 0;
    std::vector<Skeletons> skeleton_result;
    Skeletons skeleton_tmp;
    ...
    while (result == K4A_STREAM_RESULT_SUCCEEDED)
    {
        ...
        if (result == K4A_STREAM_RESULT_SUCCEEDED)
        {
            ...
            if (pop_frame_result == K4A_WAIT_RESULT_SUCCEEDED)
            {
                ...
                // Put skeleton data into struct
                skeleton_tmp.NumSkeletons = num_bodies;
                if (inputSettings.SaveSkeleton)
                {                    
                    for (size_t i = 0; i < num_bodies; i++)
                    {                            
                        // get skeletons info                    
                        k4abt_frame_get_body_skeleton(bodyFrame, i, &skeleton_tmp.body.skeleton);
                        skeleton_tmp.body_index_map = k4abt_frame_get_body_index_map(bodyFrame);
                        skeleton_tmp.body.id = k4abt_frame_get_body_id(bodyFrame, i);
                        // push back to skeleton vector                        
                        skeleton_result.push_back(skeleton_tmp);
                    }
        ...
        if (result == K4A_STREAM_RESULT_EOF)
        {
            ...
            if (inputSettings.SaveSkeleton)
            {
                ...
                const char* out_file = inputSettings.OutName.c_str();                
                write_file.open(out_file);
                if (write_file.is_open())
                {
                    write_file << num_frames << std::endl;
                    for (size_t i = 0; i < num_frames; i++)
                    {                                                
                        write_file << skeleton_result[i].NumSkeletons << std::endl;
                        write_file << skeleton_result[i].body.id << ' ' << skeleton_result[i].ClipedEdge << ' '
                        << skeleton_result[i].HandLeftConfidence << ' '
                        << skeleton_result[i].HandLeftState << ' '
                        << skeleton_result[i].HandRighttConfidence << ' '
                        << skeleton_result[i].HandRighttState << ' '
                        << skeleton_result[i].isResticted << ' '
                        << skeleton_result[i].LeanX << ' ' << skeleton_result[i].LeanY << ' '
                        << std::endl;
                        write_file << skeleton_result[i].NumJoints << std::endl;

                        for (int j = 0; j < skeleton_result[i].NumJoints; j++)
                        {
                            write_file << skeleton_result[i].body.skeleton.joints[j].position.v[0] << ' '
                            << skeleton_result[i].body.skeleton.joints[j].position.v[1] << ' '
                            << skeleton_result[i].body.skeleton.joints[j].position.v[2] << ' '
                            << skeleton_result[i].body.skeleton.joints[j].orientation.v[0] << ' '
                            << skeleton_result[i].body.skeleton.joints[j].orientation.v[1] << ' '
                            << skeleton_result[i].body.skeleton.joints[j].orientation.v[2] << ' '
                            << skeleton_result[i].body.skeleton.joints[j].orientation.v[3] << ' '
                            << skeleton_result[i].body.skeleton.joints[j].confidence_level << std::endl;
                        }
